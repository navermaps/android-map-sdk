...
- 마커 캡션에 이모지가 포함되면 의도되지 않은 위치에서 줄바꿈이 일어나는 현상 수정
- 커스텀 스타일링 적용 후 심벌을 피킹하면 크래시가 발생하는 문제 수정
...
- 네이버 클라우드 플랫폼 키 ID 인증 방식 지원
  ...
  - `com.naver.maps.map.NCP_KEY_ID` 메타데이터
- 커스텀 스타일링 기능 추가
  - `NaverMapOptions.customStyleId()`
  - `NaverMap#customStyleId`
